...
public class ParameterConstraintValidator extends AbstractExecutableConstraintValidator {
    ...
    public void validate(ProcessingEnvironment procEnv, RoundEnvironment roundEnv, TypeElement annotationType, AnnotationMirror annotation, ExecutableElement e) {
        AnnotationValue expectedReturnType = AnnotationProcessingUtil.getAnnotationElementValue(procEnv, annotation, "expectedParameterTypes");
        ...
        if (!matches(procEnv, strict, getElementTypeMirrors(e.getParameters()), getTypeMirrors((List<AnnotationValue>) expectedReturnType.getValue()))) {
    ...
    private List<? extends TypeMirror> getElementTypeMirrors(List<? extends Element> elements) {
    ...
    private boolean matches(ProcessingEnvironment procEnv, boolean strict, List<? extends TypeMirror> parameterTypes, List<? extends TypeMirror> expectedParameterTypes) {
        ...
        if (strict) {
            for (int i = 0; i < parameterTypes.size(); i++) {
                if (!procEnv.getTypeUtils().isSameType(expectedParameterTypes.get(i), parameterTypes.get(i))) {
                    return false;
                }
            }
        } else {
            for (int i = 0; i < parameterTypes.size(); i++) {
                if (!procEnv.getTypeUtils().isSubtype(expectedParameterTypes.get(i), parameterTypes.get(i))) {
                    return false;
                }
            }
        }
